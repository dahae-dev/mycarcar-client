const terms: string = `
제1조 (목적)
본 약관은 대한신차장기렌트협동조합(이하 '회사')가 대한신차장기렌트협동조합 사이트(이하 ‘사이트’)를 통하여 인터넷상에서 제공하는 서비스(이하 ‘서비스’)를 이용하는 고객 (이하 '고객')간의 권리와 의무 및 책임 등 기타 제반사항을 규정함을 목적으로 합니다.
...
const privatePolicy: string = `
엔에이치엔고도 주식회사(이하 ‘회사’)는 다음과 같은 원칙에 의하여 이용자의 개인정보를 수집, 이용 및 관리하고 있고, 이와 관련하여 정보통신서비스 제공자가 준수하여야 하는 대한민국의 관계법령 및 개인정보보호 규정, 가이드라인을 준수하고 있습니다.
...
(2) 기타 관계 법령에 의한 개인정보의 보존
 ...
 근거법령 	 개인정보의 종류 	 보존기간 
 통신비밀보호법 제15조의 2 및 동 시행령 제41조 제2항(통신사실확인자료) 	 법 제2조 제11호 가목부터 라목까지 및 바목에 따른 통신사실확인자료(가입자의 전기통신일시, 전기통신개시ㆍ종료시간, 발ㆍ착신 통신번호 등 상대방의 가입자번호, 사용도수) 	 12개월 
 법 제2조 제11호 마목 및 사목에 따른 통신사실확인자료(컴퓨터통신 또는 인터넷의 사용자가 전기통신역무를 이용한 사실에 관한 컴퓨터통신 또는 인터넷의 로그기록자료, 컴퓨터통신 또는 인터넷의 사용자가 정보통신망에 접속하기 위하여 사용하는 정보통신기기의 위치를 확인할 수 있는 접속지의 추적자료) 	 3개월 
 전자상거래등에서의 
 소비자보호에 관한 법률 
 제6조 및 동 시행령 제6조 
 (거래기록) 	 표시 광고에 관한 기록 	 6개월 
 소비자의 불만 또는 분쟁처리에 관한 기록 	 3년 
 계약 또는 청약철회 등에 관한 기록,
 대금결제 및 재화 등의 공급에 관한 기록 	 5년 
 국세기본법 제85조의 3 	 납세자의 납세 근거자료 등 (모든 거래에 관한 장부 및 증거서류)
 5년 
 상법 제33조 	 상업장부와 영업에 관한 중요서류 	 10년 
 전표 또는 이와 유사한 서류 	 5년 
 국세기본법 제85조의 3 	 모든 거래에 관한 장부 및 증거서류: 그 거래사실이 속하는 과세기간에 대한 해당 국세의 법정신고기한이 지난 날부터 5년간 	 5년 
...
- 이용자가 사전에 동의한 경우
 ...
 ㈜엔에이치엔에이스	 쇼핑몰 통계 서비스	 이름, 이메일, 전화번호 	 서비스 해지 후 파기 
...
회사는 경영 효율성의 제고, 서비스 품질 향상 등을 위하여 회사 업무의 일부를 외부 전문업체 등 제3자에게 용역을 주어 수행하며 그 과정에서 제3자에게 이용자의 개인정보를 수집, 보관, 처리, 이용, 제공, 관리, 파기 등을 할 수 있도록 업무를 처리위탁하게 되는데, 그에 따른 수탁자 및 위탁업무의 내용은 다음과 같습니다.
 위탁받는 자(수탁자) 	 위탁업무 내용 	 보유 및 이용기간 
 나이스 신용평가정보㈜ 	 실명확인/아이핀(I-Pin)서비스 	 위탁계약 종료시까지 
 ㈜LG U+, ㈜세틀뱅크, NICE정보통신㈜, ㈜케이지이니시스, 엔에이치엔한국사이버결제㈜, ㈜올엣, 한국정보통신주식회사, 엔에이치엔페이코㈜	 쇼핑몰 결제 서비스 및 에스크로서비스	 위탁계약 종료시까지 
 (주)컨프론티어, ALLPAY, 페이레터(주) 	 해외 결제 서비스 	 위탁계약 종료시까지 
 KG모빌리언스, 다날, 갤러시아커뮤니케이션즈(주) 	 휴대폰 결제 서비스 	 위탁계약 종료시까지 
 ㈜엔에이치엔에이스 	 접속통계 서비스 	 위탁계약 종료시까지 
 엔에이치엔한국사이버결제㈜ 	 휴대폰 본인인증 서비스 	 위탁계약 종료시까지 
 ㈜아이네임즈, ㈜가비아, ㈜닷네임 	 도메인서비스 	 위탁계약 종료시까지 
 엔에이치엔고도 주식회사 고도아카데미, 고도몰 창업학원 	 아카데미 오프라인 교육 서비스 	 위탁계약 종료시까지 
 ㈜한국기업보안, ㈜한국정보인증 	 보안서버 SSL 인증서비스 	 위탁계약 종료시까지 
 지메이트 주식회사 	 080 수신거부 서비스 	 위탁계약 종료시까지 
 엔에치엔엔터테인먼트㈜ 	 클라우드 프로젝트 생성/상세 목록 	 위탁계약 종료시까지 
...
라. 접속기록의 보관 및 위변조 방지
개인정보처리시스템에 접속한 기록을 최소 6개월 이상 보관, 관리하고 있으며, 접속 기록이 위변조 및 도난, 분실되지 않도록 관리하고 있습니다.
마. 개인정보의 암호화
...
- 개인정보 분쟁조정위원회 (국번없이)118 (내선2번), http://privacy.kisa.or.kr
- 경찰청 사이버안전국 (국번 없이)182, http://www.ctrc.go.kr
- 대검찰정 사이버수사과 (국번없이) 1301, www.spo.go.kr
...
11. 고지의무
본 개인정보 처리방침은 2018년 7월 19일에 제정되었으며 정부의 정책 또는 보안기술의 변경에 따라 내용의 추가, 삭제 및 수정이 있을 시에는 개정 최소 7일 전부터 홈페이지의 ‘공지사항’을 통해 고지할 것입니다.
`;

export { terms, privatePolicy };
